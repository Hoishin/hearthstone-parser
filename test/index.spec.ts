...
describe("hearthstone-log-watcher", () => {
	describe("constructor", () => {
		...
		it("should override the options with passed in values.", () => {
			...
			logWatcher.#options.should.have.property(
				"configFile",
				configFileFixture
			);
		});
	...
	describe("parsing", () => {
		...
		it("should correctly handle a togwaggle deck swap", () => {
			const logFilePath = path.join(
				__dirname,
				"/artifacts/togwaggle_deck_swap.log"
			);
			const logWatcher = new LogWatcher({
				logFile: logFilePath,
				configFile: configFileFixture,
		...
		it("should not update turn start time until new turn animation", () => {
			const logFilePath = path.resolve(
				__dirname,
				"artifacts/long-animation-end-of-turn.log"
			);
			const logWatcher = new LogWatcher({
				logFile: logFilePath,
				configFile: configFileFixture,
		...
		it("should correctly handle discovery of bottom", () => {
			const logFilePath = path.resolve(
				__dirname,
				"artifacts/discovery-bottom.log"
			);
			const logWatcher = new LogWatcher({
				logFile: logFilePath,
				configFile: configFileFixture,
		...
		it("should correctly handle discovery with Spirit of the Shark", () => {
			const logFilePath = path.resolve(
				__dirname,
				"artifacts/discovery-double.log"
			);
			const logWatcher = new LogWatcher({
				logFile: logFilePath,
				configFile: configFileFixture,
		...
		it("should correctly handle a new game", () => {
			...
			expect(gameState.players[0].position).not.to.equal(
		...
		it("should correctly count cards replaced in mulligan", () => {
			const logFilePath = path.resolve(
				__dirname,
				"artifacts/mulligan-card-count.log"
			);
			const logWatcher = new LogWatcher({
				logFile: logFilePath,
				configFile: configFileFixture,
			...
			// 2, 4
			expect(
				gameState.getPlayerByPosition("bottom")!.cardsReplacedInMulligan
			).to.equal(2);
			expect(
				gameState.getPlayerByPosition("top")!.cardsReplacedInMulligan
			).to.equal(4);
		});
		...
		const testMatchLog = (artifact: string) => {
			it(`handles matchlog ${artifact}`, () => {
				...
				const resultPath = path.resolve(
					__dirname,
					`artifacts/match-log-results/${artifact}.json`
				);
...
describe("Gamestate", () => {
	describe("addPlayer", () => {
		it("should update names", () => {
			...
			state
				.getPlayerById(2)
				?.name.should.equal("world", "expected name to update");
		});
	});
});
