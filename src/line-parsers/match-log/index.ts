...
import { GameState, MatchLogEntry } from "../../game-state";
...
export class MatchLogParser extends LineParser {
	 ...
	 */
	private _handleMatchLogTopLevelBlock(
		emitter: HspEventsEmitter,
		gameState: GameState,
		block: BlockData
	) {
		...
		if (
			...
			validTriggerTypes.includes(block.triggerKeyword ?? "")
		) {
		...
		const proposedDefender = filterTags(
			block.entries,
			"PROPOSED_DEFENDER"
		)[0]?.value;
		const mainTarget =
			block.blockType === "ATTACK" && proposedDefender
		...
		const handleTrigger = (trigger: BlockData) => {
			if (
				!isCard(trigger.entity) ||
				!validTriggerTypes.includes(trigger.triggerKeyword ?? "")
			) {
				return;
			}
			...
			if (
				logEntry.type === "play" &&
				trigger.triggerKeyword !== "DEATHRATTLE"
			) {
			...
			const triggerLogEntry = new MatchLogEntry(
				"trigger",
				trigger.entity
			);
			let addBefore = false;
		...
		const handlePower = (power: BlockData) => {
			...
			for (const subEntry of entries) {
				...
				if (
					subEntry.type === "block" &&
					subEntry.blockType === "POWER" &&
					isCard(subEntry.entity)
				) {
		...
		for (const entry of block.entries) {
			...
			if (
				entry.type === "embedded_entity" &&
				entry.action === "Creating" &&
				entry.entity.tags.ZONE === "HAND"
			) {
			...
			if (entry.type === "block") {
				...
				if (entry.blockType === "DEATHS") {
					...
					for (const death of deaths.values()) {
						mostRecent.addTarget(mainContext.get(death), {
							dead: true,
						});
					}
				}
	...
	private _handleTopLevelDeaths(gameState: GameState, block: BlockData) {
		...
		for (
			let i = gameState.matchLog.length - 1;
			i >= 0 && deaths.size;
			i--
		) {
	 ...
	 */
	private _handleTopLevelTrigger(
		emitter: HspEventsEmitter,
		...
		attacks.forEach((a) =>
			this._handleMatchLogTopLevelBlock(emitter, gameState, a)
		);
	 ...
	 * @param line
	 */
	private _handleHealthChange(
		logEntry: MatchLogEntry,
		context: BlockContext,
		line: Entry
	) {
		...
		if (values) {
			const entries = [logEntry.source, ...logEntry.targets].filter(
				(p) => p.entityId === values.entityId
			);
			if (!entries.length) {
				logEntry.addTarget(
					entities.get(values.entityId),
					values.values
				);
				return;
			}
			...
			for (const entry of entries) {
				...
				if (values.values.healing) {
					entry.healing =
						(entry.healing ?? 0) + values.values.healing;
				}
			}
		}
	}
	...
	private _resolveTarget(
		...
		const entityId =
			"entity" in entry && isCard(entry.entity)
		...
		if (entry.type === "embedded_entity" && entry.action === "Updating") {
			...
			const hasCreation = entries.find(
				(e) =>
					e.type === "embedded_entity" &&
					e.action === "Creating" &&
		...
		if (testTag(entry, "ZONE_POSITION") && isCard(entry.entity)) {
			const zoneTag = filterTags(entries, "ZONE").find(
				(e) => isCard(e.entity) && e.entity.entityId === entityId
			);
			const embedded = entries.find(
				(e) =>
					e.type === "embedded_entity" &&
					e.entity.entityId === entityId
			) as FullEntity | undefined;
		...
		if (testTag(entry, "SPAWN_TIME_COUNT") && isCard(entry.entity)) {
			...
			if (
				existing &&
				existing.tags.CARDTYPE === "ENCHANTMENT" &&
				existing.tags.ATTACHED
			) {
	 ...
	 * @param deathBlock
	 */
	private _resolveDeaths(
		deathBlock: BlockData | null | undefined
	): Set<number> {
